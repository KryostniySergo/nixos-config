...
#+BEGIN_SRC sh :noeval
# Install from gitlab
nix run gitlab:KryostniySergo/nixos-config
...
and then running:
#+BEGIN_SRC sh :noeval
nix run --experimental-features 'nix-command flakes' gitlab:KryostniySergo/nixos-config
#+END_SRC

And if you want a single copy-paste solution:
#+begin_src sh :noeval
nix-shell -p git --command "nix run --experimental-features 'nix-command flakes' gitlab:KryostniySergo/nixos-config"
#+end_src
